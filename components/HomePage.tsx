...
const HomePage: React.FC<HomePageProps> = ({ onNavigate, navItems }) => {
  return (
    <div className="flex flex-col items-center justify-center h-full p-4 sm:p-6 bg-gray-800 rounded-lg shadow-xl text-center">
      <HPDSKLogoIcon className="w-20 h-20 sm:w-24 sm:h-24 mb-6" />
      <h1 className="text-3xl sm:text-4xl font-bold text-purple-400 mb-4">Bem-vindo ao HPDSK!</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 w-full max-w-4xl mt-8">
        {navItems.map(item => (
          <button
            ...
            className={`${item.colorClass || 'bg-purple-500 hover:bg-purple-600'} text-white font-medium py-3 px-4 rounded-lg shadow-md hover:shadow-lg transition-all duration-150 ease-in-out flex items-center justify-center text-sm sm:text-base`}
      ...
      <p className="text-xs sm:text-sm text-gray-500 mt-12">
